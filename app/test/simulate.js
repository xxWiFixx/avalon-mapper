...
(async () => {
  ...
  const EXTRA = [
    ...
    // Четвёртый — тултип ПОВЕРХ МИНИКАРТЫ, и в этом всё дело. Песчаная текстура карты
    // попадает в тот же коричнево-золотой диапазон, что и тёмная дорожка полосы, а
    // промежутки между ними мельче мостика: полоса склеивалась с картой в один отрезок,
    // тот перерастал потолок длины и выбрасывался целиком. От тултипа не оставалось ни
    // одного кандидата — над картой портал не читался никогда. Кадр игрока от 29 июля.
    ['c4-minimap-7-7', { name: 'Conos-Avaelum', capNum: 7, capMax: 7, closes: 10 * 3600 + 1 * 60 }, 1080],
    // Пятый — тултип поверх БОЛЬШОЙ КАРТЫ ЗОНЫ, и он про другую половину той же беды.
    // Песок карты проходит как дорожка полосы не только справа, но и СЛЕВА: отрезок
    // начинался далеко левее полосы, bx уезжал вместе с ним, и имя читалось мимо
    // тултипа. Лечится тем, что левый край берётся по заливке (она начинается ровно
    // на краю полосы), а не по началу склеенного отрезка. Кадр игрока от 29 июля.
    ['c5-zonemap-5-7', { name: 'Xasos-Aeoilos', capNum: 5, capMax: 7, closes: 11 * 3600 + 11 * 60 }, 1080],
    // Шестой — тултип на миникарте посреди ЗОЛОТОЙ ТРАВЫ. Трава и тени в ней проходят
    // фильтры полосы десятками (на этом кадре кандидатов 50), все они «темнее» настоящей
    // полосы, и одиночный лучший кандидат всегда оказывался тенью — портал не читался,
    // пока курсор не сдвинут. Лечится двумя вещами: кандидат с заливкой правдоподобной
