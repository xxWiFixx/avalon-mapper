...
t('стандартное место: над миникартой, по левому краю плашки зоны', () => {
  ...
  // Высоту НЕ сверяем с числом: это запас прозрачного места сверху, он растёт вместе с
  // содержимым (появился проводник по маршруту — стало 580 вместо 470), и тест, который
  // просто повторял бы константу, ловил бы не ошибку, а сам факт правки. Проверяем то,
  // ради чего запас и нужен: его хватает на самое высокое содержимое.
  if (!(b.height >= 470)) throw new Error('высоты не хватит на плашку с картой: ' + b.height);
  // низ блока = верх плашки − 267 (до компаса) − 12 (зазор). ВОТ ЭТО и есть договор с
  // игроком: сколько бы места ни занимал блок, его нижний край стоит над миникартой.
